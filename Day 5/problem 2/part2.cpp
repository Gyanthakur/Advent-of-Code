...
// Function to sort a single update based on the rules
void sortUpdate(vector<int>& update, const unordered_map<int, unordered_set<int>>& rules) {
    sort(update.begin(), update.end(), [&rules](int a, int b) {
        ...
        return a < b; // Default to ascending order if no explicit rule
    });
}
...
int main() {
    ...
    cout << "Sum of middle pages for fixed updates: " << sumMiddlePages << endl;
